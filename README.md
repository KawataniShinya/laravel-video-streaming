...
- **419 Page Expired 対策**: ログイン画面でのセッションタイムアウト時、自動的にログイン画面へリダイレクトして CSRF トークンを更新するようユーザー体験を改善しています
...
- **ハッシュ値の表示**: サーバー上のディレクトリ名（ハッシュ）を表示します。クリックすることでフル桁のハッシュ値をクリップボードにコピー可能です
- キャッシュ単位で削除できます
- 複数選択削除と全削除もできます
...
- `ffmpeg.pid` が生きていれば `Transcoding`。進捗率（%）とプログレスバーが表示されます
...
- **アクセシビリティ**: テーブルヘッダーは `aria-sort` に対応し、キーボード操作可能なボタンとして実装されています
