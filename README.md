...
- 管理者は HLS キャッシュの状態、サイズ、ディスク残量を確認できます
- キャッシュ単位で削除できます
- 複数選択削除と全削除もできます
...
- HLS キャッシュも対象に含めます
- VOB の冗長ファイルは `VTS_01_1.VOB` 以外を整理対象として扱います
...
- そのまま自分の `Movies` フォルダを使うなら、サンプルの `~/Movies:/videos` を使えます
- リポジトリ配下に動画を置きたいなら、`./videos:/videos` のコメントを外して使えます
